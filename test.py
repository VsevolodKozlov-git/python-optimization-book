...
from tools import get_solution_variables, get_objective_value
...
def solve(food_creature, food_amount):
    ...
    # Для удобства запишем количество типов животных и типов еды в переменны
    ...
    for food_i in range(food_n):
        ...
        food_i_eaten = s.Sum(creat_x[creat_j] * food_creature[food_i][creat_j] for creat_j in range(creat_n))
        s.Add(food_i_eaten <= food_amount[food_i])
    # Решаем задачу
